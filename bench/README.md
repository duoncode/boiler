...
Run the benchmark with Xdebug and PCOV disabled. Both add substantial
runtime overhead, especially for Boiler's proxy-based auto escaping, so
results with either extension enabled are not useful for fair engine
comparisons. The benchmark script warns when it detects either of them.
`composer benchmark` already runs it with `xdebug.mode=off` and
`pcov.enabled=0`.
